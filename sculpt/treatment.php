...
<div class="container" style="background-color: #FCF6EF;">
      ...
      <img class="topRight" src="<?php echo get_template_directory_uri(); ?>/images/aboutTopLeft.png" alt="Dr Deepa Panch">
        ...
        <div class="wrapper">
            ...
            <div class="titleWrapper" style="transform:translateX(50px);opacity:0;">
                <h1 class="largeh1"><?php the_title(); ?></h1>
            </div>
            <div class="imageWrapper" style="transform:translateX(-50px);opacity:0;">
              ...
              <?php
                  if ( has_post_thumbnail() ) {
                      ...
                      // Add the 'textOverlay' image size to the_post_thumbnail() function
                      the_post_thumbnail( 'textOverlay', $thumbnail_attr );
                  }
              ?>
            </div>
            <div class="downButtonWrapper" style="transform:translateY(-50px);opacity:0;">
                <a class="downButton" href="#next">
                    <svg xmlns="http://www.w3.org/2000/svg" width="50" height="103" viewBox="0 0 50 103"><g transform="translate(-1237 -646)"><g transform="translate(532.5 -28.501)"><line y2="37.103" transform="translate(730 707.501)" fill="none" stroke="#ae483d" stroke-width="2"/><path d="M722.565,733.673l5.357,5.31,5.357-5.31" transform="translate(2.077 5.97)" fill="none" stroke="#ae483d" stroke-width="2"/></g><g transform="translate(1237 646)" fill="none" stroke="#ae483d" stroke-width="2"><rect width="50" height="103" rx="25" stroke="none"/><rect x="1" y="1" width="48" height="101" rx="24" fill="none"/></g></g></svg>
                </a>
            </div>
        </div>
    </section>
    ...
    <?php if( get_field('pricelist_title') ) { ?>
    <section class="pricesIntro">
        <div class="wrapper">
            ...
            <h2 class="onScreen"><?php the_field('pricelist_title'); ?></h2>
      ...
      gsap.registerPlugin(ScrollTrigger);
        ...
        [largedesktopBP]: function() {
          var tlHome = gsap.timeline({ease: "power3"})
            ...
            let logo = document.querySelector('.titleImage .wrapper .logoWrapper');
            let titleWrapper = document.querySelector('.titleImage .titleWrapper');
            let image = document.querySelector('.titleImage .imageWrapper');
            let downButtonWrapper = document.querySelector('.treatments .wrapper .text .bottomText');
            let overview = document.querySelectorAll('.overview .leftText');
            let overviewItems = document.querySelectorAll('.overviewItem');

            tlHome.to(nav, {duration: 0.5, y:0, opacity:1, delay: 0.1})
            // tlHome.to(logo, {opacity:1, y:0, duration: .5}, "-=.5")
            tlHome.to(titleWrapper, {x:0, duration: 0.5, opacity:1}, "-=.5")
            tlHome.to(image, {x:0, duration: 0.5, opacity:1}, "-=.5")
            tlHome.to(downButtonWrapper, {y:0, duration: 0.5, opacity:1}, "-=.5")
            tlHome.to(overview, {y:0, duration: 0.5, opacity:1}, "-=.5")
            tlHome.to(overviewItems, {x:0, duration: 0.5, opacity:1, stagger: .15}, "-=.5")
        },
